...
@php
    $isFreePterodactyl = $order->package && $order->package->service === 'freepterodactyl';
    $daysUntilExpiration = now()->diffInDays($order->due_date, false);
    
    if ($isFreePterodactyl) {
        $canRenew = $daysUntilExpiration <= 1;
        $renewThreshold = 1;
    } else {
        $canRenew = $daysUntilExpiration <= 15;
        $renewThreshold = 15;
    }
@endphp
...
<button type="button" 
    @if($canRenew) 
        data-modal-target="renewService-{{ $order->id }}" data-modal-toggle="renewService-{{ $order->id }}"
    @else
        onclick="showRenewAlert({{ $daysUntilExpiration }}, {{ $renewThreshold }}, {{ $isFreePterodactyl ? 'true' : 'false' }})"
    @endif
...
<script>
    ...
    function showRenewAlert(daysUntilExpiration, renewThreshold, isFreePterodactyl) {
        let alertMessage;
        let thresholdText = renewThreshold === 1 ? '1 day' : `${renewThreshold} days`;
        
        if (daysUntilExpiration > renewThreshold) {
            if (isFreePterodactyl) {
                alertMessage = `You can only renew your free service when there is less than ${thresholdText} left before it expires. Your service expires in ${daysUntilExpiration} days.`;
            } else {
                alertMessage = `You can only renew your service when there is less than ${thresholdText} left before it expires. Your service expires in ${daysUntilExpiration} days.`;
            }
        } else {
            if (isFreePterodactyl) {
                alertMessage = `You can only renew your free service when there is less than ${thresholdText} left before it expires.`;
            } else {
                alertMessage = `You can only renew your service when there is less than ${thresholdText} left before it expires.`;
            }
        }
        
        // Create alert modal
        const alertModal = document.createElement('div');
        alertModal.className = 'fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50';
        alertModal.innerHTML = `
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
                <div class="flex items-center mb-4">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Renewal Restricted</h3>
                </div>
                <p class="text-gray-600 dark:text-gray-400 mb-6">${alertMessage}</p>
                <div class="flex justify-end">
                    <button onclick="this.closest('.fixed').remove()" class="bg-primary-700 hover:bg-primary-800 text-white px-4 py-2 rounded-lg">
                        OK
                    </button>
                </div>
            </div>
