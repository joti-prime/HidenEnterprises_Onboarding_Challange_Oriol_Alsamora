...
@php
    // Check if this service has configurable options - if so, don't show traditional upgrade
    $hasConfigurableOptions = false;
    $packageOptions = [];
    
    if (strtolower($order->package->service) === 'pterodactyl' && $order->status === 'active') {
        $packageOptions = \App\Rules\ConfigurableOptionLimits::getPackageOptions($order->package_id);
        $hasConfigurableOptions = !empty($packageOptions);
    }
...
@if (request('page') == 'manage' && !$hasConfigurableOptions)
