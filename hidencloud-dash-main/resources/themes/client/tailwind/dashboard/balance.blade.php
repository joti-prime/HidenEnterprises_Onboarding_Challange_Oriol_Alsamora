...
@section('keywords', 'HCTestDash Dashboard, HCTestDash Panel')
                                    ...
                                    <tr class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            @if ($transaction->result == '+')
                                                <span class="inline-flex items-center bg-green-100 text-green-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">
                                                    <span class="mr-1">{{ $transaction->result }}</span>
                                                    <span>{{ price($transaction->amount) }}</span>
                                                </span>
                                            @elseif($transaction->result == '-')
                                                <span class="inline-flex items-center bg-red-100 text-red-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">
                                                    <span class="mr-1">{{ $transaction->result }}</span>
                                                    <span>{{ price($transaction->amount) }}</span>
                                                </span>
                                            @elseif($transaction->result == '=')
                                                <span class="inline-flex items-center bg-gray-100 text-gray-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">
                                                    <span class="mr-1">{{ $transaction->result }}</span>
                                                    <span>{{ price($transaction->amount) }}</span>
                                                </span>
                                            @endif
